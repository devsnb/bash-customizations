...
# ── Cached tool capabilities ──────────────────────────────────────────────────
# setup.sh resolves these once so a missing optional tool does not make every
# new WSL shell search a long PATH containing slow /mnt/c entries.  If the
# cache is absent or stale-versioned, _bc_has safely falls back to command -v.
_bc_capability_cache="${XDG_CACHE_HOME}/bash-customizations/capabilities.sh"
if [[ -r "$_bc_capability_cache" ]]; then
    IFS= read -r _bc_capability_header < "$_bc_capability_cache" || true
    if [[ "$_bc_capability_header" == '# Generated by bash-customizations setup.sh; do not edit.' ]]; then
        # shellcheck source=/dev/null
        source "$_bc_capability_cache"
    fi
fi
unset _bc_capability_cache _bc_capability_header

# _bc_has TOOL — trust cached misses and cheaply confirm cached hits.
# Positive results are rechecked because PATH can differ between terminal
# sessions; found commands normally resolve near its front. Keep this private
# helper available because later modules and .blerc share it.
_bc_has() {
    local variable="BC_CAP_${1^^}"
    if [[ "${BC_CAP_CACHE_VERSION:-}" == 1 ]] \
        && declare -p "$variable" &>/dev/null; then
        [[ "${!variable}" == 0 ]] && return 1
    fi
