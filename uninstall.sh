...
read_manifest() {
    ...
    while IFS='=' read -r key value; do
        ...
        case "$key" in
            ...
            TOOL)
                if [[ "$value" =~ ^(starship|fd|fzf|zoxide|blesh):[^:]+:[0-9a-f]{64}$ ]]; then
...
purge_tools() {
    ...
    for record in "${MANIFEST_TOOLS[@]}"; do
        name="${record%%:*}"
        case "$name" in
            starship|fd|fzf|zoxide) echo "  ~/.local/bin/${name}" ;;
    ...
    for name in starship fd fzf zoxide; do
...
# remove_capability_cache — remove only the exact generated file we recognise.
# A user-created file at the same path is left alone rather than guessed at.
remove_capability_cache() {
    [[ -e "$CAPABILITY_CACHE_FILE" || -L "$CAPABILITY_CACHE_FILE" ]] || return 0

    local header=""
    if [[ ! -L "$CAPABILITY_CACHE_FILE" && -f "$CAPABILITY_CACHE_FILE" ]]; then
        IFS= read -r header < "$CAPABILITY_CACHE_FILE" || true
    fi
    if [[ "$header" != '# Generated by bash-customizations setup.sh; do not edit.' ]]; then
        log_warn "Capability cache is not recognised — leaving it untouched: ${CAPABILITY_CACHE_FILE}"
        return 0
    fi

    run rm "$CAPABILITY_CACHE_FILE"
    report "Removed runtime cache: ${CAPABILITY_CACHE_FILE}" \
           "Would remove runtime cache: ${CAPABILITY_CACHE_FILE}"
    if ! $DRY_RUN; then
        rmdir "$CAPABILITY_CACHE_DIR" 2>/dev/null || true
    fi
}
...
main() {
    ...
    if ! $DRY_RUN; then
        ...
        if $RESTORE_ONLY; then
            ...
            [[ -e "$CAPABILITY_CACHE_FILE" ]] \
                && echo "  Remove the generated runtime capability cache"
        fi
        echo
